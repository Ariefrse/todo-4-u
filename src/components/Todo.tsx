...
const Todo = () => {
  ...
  const addTodo = () => {
    if (todo.trim() !== "") {
        const newTask = { id: Date.now(), task: todo, completed: false, lastUpdated: new Date().toLocaleString() };
  ...
  const toggleTodo = (id: number) => {
    const updatedTasks = task.map((item) =>
        item.id === id ? { ...item, completed: !item.completed, lastUpdated: new Date().toLocaleString() } : item // Update this line
    );
    updateLocalStorage(updatedTasks);
};
